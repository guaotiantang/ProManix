...
class NDSPool:
    ...
    async def _acquire_lock_with_timeout(self, lock: asyncio.Lock, timeout: float = 5.0) -> bool:
        """尝试在指定时间内获取锁"""
        try:
            print(f"Trying to acquire lock...")
            await asyncio.wait_for(lock.acquire(), timeout)
            print(f"Lock acquired")
            return True
        except asyncio.TimeoutError:
            print(f"Lock acquire timeout")
            return False
    ...
    async def _cleanup_idle_connections(self) -> None:
        """清理空闲连接"""
        print("Starting cleanup idle connections...")
        # 先获取所有需要处理的服务器ID
        async with self._global_lock:
        ...
        for server_id in server_ids:
            try:
                lock = await self._get_server_lock(server_id)
                if not await self._acquire_lock_with_timeout(lock):
                    print(f"Skip cleanup for server {server_id} due to lock timeout")
                    continue
                ...
                try:
                    ...
                    lock.release()
                    print(f"Cleanup lock released for server {server_id}")
            except Exception as e:
                logger.error(f"Error cleaning up connections for server {server_id}: {e}")

    async def _release_connection(self, server_id: str, conn: ConnectionInfo) -> None:
        """释放连接（内部使用）"""
        print(f"Trying to release connection for server {server_id}...")
        lock = await self._get_server_lock(server_id)
        
        if not await self._acquire_lock_with_timeout(lock):
            print(f"Warning: Failed to acquire lock for releasing connection of server {server_id}")
            # 即使获取锁失败，也要尝试标记连接为未使用
            conn.in_use = False
            conn.last_used = datetime.now()
            return

        try:
            print(f"Got lock for releasing connection of server {server_id}")
            conn.in_use = False
            conn.last_used = datetime.now()
        finally:
            lock.release()
            print(f"Released lock for server {server_id}")

    @asynccontextmanager
    async def get_client(self, server_id: str):
        """获取客户端连接的上下文管理器"""
        conn = await self._get_connection(server_id)
        print(f"Client[{server_id}] Connect")
        try:
            yield conn.client
        finally:
            print(f"Client[{server_id}] Releasing...")
            await self._release_connection(server_id, conn)
            print(f"Client[{server_id}] Release")

    async def release_client(self, server_id: str, client: NDSClient) -> None:
        """手动释放客户端连接"""
        lock = await self._get_server_lock(server_id)
        async with lock:
            for conn in self._pools[server_id]:
