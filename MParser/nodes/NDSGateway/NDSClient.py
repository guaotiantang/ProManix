...
class NDSClient:
    ...
    async def scan(self, scan_path: str, filter_pattern: Optional[str] = None) -> List[str]:
        ...
        if not await self.file_exists(scan_path):
            raise NDSError(f"Path not found: {scan_path}", level=1)
    ...
    async def read(self, size: Optional[int] = None, offset: Optional[int] = None) -> bytes:
        ...
        async with self._lock:
            if not self.client:
                raise NDSIOError("Client not initialized", level=-1)
            if not self.stream_path:
                raise NDSFileNotFoundError("No file opened", level=0)
            ...
            try:
                if self.protocol == "FTP":
                    return await self._read_ftp(size)
                elif self.protocol == "SFTP":
                    return await self._read_sftp(size)
                else:
                    raise NDSError("Invalid protocol, only support FTP and SFTP", level=1)
            except Exception as e:
                raise NDSIOError(f'Read error: {str(e)}', level=-1)

    async def _read_ftp(self, size: Optional[int]) -> bytes:
        """FTP文件读取实现
        ...
        Raises:
            NDSIOError: 读取过程中发生错误
        """
        stream = await self.client.get_stream("RETR " + self.stream_path, "1xx",
        ...
        self.__stream_offset += size
        return tmp_io.getvalue()[0:size]

    async def _read_sftp(self, size: Optional[int]) -> bytes:
        """SFTP文件读取实现
    ...
    async def get_zip_info(self, file_path: str = None) -> list[KeyType[Any, Any]]:
        ...
        if len(tmp_data) != header_size or header_data[0] != ZIP_MAGIC:
            raise NDSZipError("Invalid ZIP header")
        ...
        if len(tmp_data) != cd_end_size or tmp_data[0:4] != ZIP_END_MAGIC or tmp_data[-2:] != b"\000\000":
            raise NDSZipError("Invalid ZIP central directory")
        ...
        if cd_end_size + cd_end_l64_size < self.stream_info['size']:
            ...
            if len(tmp_data) == cd_end_l64_size:
                sig, disk_no, _, disks = struct.unpack(cd_end_l64_struct, tmp_data)
                if sig == ZIP64_LOCATOR:
                    ...
                    if len(tmp_data) == cd_end_64_size:
                        sig, _, _, _, disk_num, disk_dir, dir_count, dir_count2, dir_size, dir_offset = \
        ...
        while total < cd_end_size:
            ...
            if len(centdir) != cd_dir_size:
                raise NDSZipError("Truncated central directory")
            centdir = struct.unpack(cd_dir_struct, centdir)
            if centdir[0] != ZIP_DIR_MAGIC:  # _CD_SIGNATURE = 0
                raise NDSZipError("Bad magic number for central directory")
            ...
            #  使用read跳过部分无需读取的数据息
            # _CD_EXTRA_FIELD_LENGTH = 13 // 外字段信息
